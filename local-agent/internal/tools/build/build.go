...
func Register(reg *tools.Registry, deps Deps) {
	...
	reg.Register(ToolRun, func(ctx context.Context, req *messages.ToolRequestPayload) (*messages.ToolResponsePayload, error) {
		...
		if adapter == nil {
			return fail(req.CallID, protoerr.BuildToolNotFound, "no build adapter detected for workspace"), nil
		}
		result, err := adapter.Run(ctx, build.Request{
		...
		if result.Status != "SUCCESS" {
			...
			case "TIMEOUT":
				e, _ := protoerr.New(protoerr.ProcessTimeout, "build timed out")
				resp.Error = e
			case "CANCELLED":
				e, _ := protoerr.New(protoerr.ProcessCancelled, "build cancelled")
				resp.Error = e
			default:
				e, _ := protoerr.New(protoerr.BuildFailed, "build failed")
				resp.Error = e
			}
		}
		if resp.Metrics == nil {
